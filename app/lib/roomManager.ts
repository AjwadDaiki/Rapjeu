...
export class RoomManager {
  ...
  private async initEncheres(room: Room): Promise<void> {
    let theme = null;
    // Retry up to 10 times to find a theme with at least 4 valid answers
    for (let i = 0; i < 10 && !theme; i++) {
      const themes = await themeService.getRandomThemes(1);
      if (themes[0] && themes[0].validAnswers && themes[0].validAnswers.length >= 4) {
        theme = themes[0];
      }
    }
    if (!theme) {
