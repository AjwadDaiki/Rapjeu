    ...
    <div className="flex flex-col items-center justify-center h-full p-8" style={{ background: 'var(--rj-bg)' }}>
      ...
      <div className="flex items-center justify-center gap-4 mb-4 text-xs">
        <div className="flex items-center gap-1.5">
          ...
          <span style={{ color: 'var(--rj-text-muted)' }}>Correct</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded" style={{ background: '#F59E0B' }} />
          <span style={{ color: 'var(--rj-text-muted)' }}>Proche</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded" style={{ background: '#DC2626' }} />
          <span style={{ color: 'var(--rj-text-muted)' }}>Faux</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span style={{ color: 'var(--rj-text-muted)' }}>{'\u2B06\u2B07'} = plus haut / plus bas</span>
        </div>
      </div>
      ...
      <div className="mb-8 w-full max-w-4xl">
        ...
        {/* Empty Slots - show max 2 to avoid intimidating grid */}
        {Array.from({ length: Math.min(2, data.maxAttempts - data.attempts.length) }).map((_, idx) => (
          <div key={`empty-${idx}`} className="grid grid-cols-6 gap-3 mb-3 opacity-20">
            <div
        ...
        {data.maxAttempts - data.attempts.length > 2 && (
          <div className="text-center text-xs mb-3" style={{ color: 'var(--rj-text-muted)' }}>
            + {data.maxAttempts - data.attempts.length - 2} tentative{data.maxAttempts - data.attempts.length - 2 > 1 ? 's' : ''} restante{data.maxAttempts - data.attempts.length - 2 > 1 ? 's' : ''}
          </div>
        )}
      </div>
